...
const generators = {
  ...
  spatial_equal_distance_3d: gen('spatial_equal_distance_3d','geom_3d_spatial_reasoning',(r,d)=>{
    const squareAxes=pick(r,[[0,1],[0,2],[1,2]]),otherAxis=[0,1,2].find(axis=>!squareAxes.includes(axis)),side=integer(r,2,size(d,4,6,8)),dimensions=[side,side,side],stretch=pick(r,d==='easy'?[1,1]:[1,1.5,2]);dimensions[otherAxis]=side*stretch;const fixed=r()<.5?0:dimensions[otherAxis],diagonal=r()<.5?[[0,0],[side,side]]:[[0,side],[side,0]],makePoint=(pair)=>{const point=[0,0,0];point[otherAxis]=fixed;point[squareAxes[0]]=pair[0];point[squareAxes[1]]=pair[1];return point;},a=makePoint(diagonal[0]),b=makePoint(diagonal[1]),vertices=[];for(const x of [0,dimensions[0]])for(const y of [0,dimensions[1]])for(const z of [0,dimensions[2]])vertices.push([x,y,z]);const key=point=>point.join(':'),distanceSquared=(p,q)=>p.reduce((sum,value,axis)=>sum+(value-q[axis])**2,0),remaining=vertices.filter(point=>key(point)!==key(a)&&key(point)!==key(b)),equal=remaining.filter(point=>distanceSquared(point,a)===distanceSquared(point,b)),unequal=remaining.filter(point=>distanceSquared(point,a)!==distanceSquared(point,b)),outlier=pick(r,unequal),candidateCoordinates=shuffle(r,[...equal,outlier]),labels=shuffle(r,['C','D','E','F','G','H']),labelByKey=new Map([[key(a),'A'],[key(b),'B']]);remaining.forEach((point,index)=>labelByKey.set(key(point),labels[index]));const answer=labelByKey.get(key(outlier)),distractors=equal.map(point=>labelByKey.get(key(point))),points=vertices.map(([x,y,z])=>({x,y,z,label:labelByKey.get(key([x,y,z]))}));
    return {prompt:'Which of the five option points is not the same distance from point A as it is from point B?',answer,distractors,explanation:`A and B are opposite corners of a square face. Four option points lie on the perpendicular-bisector plane of AB; point ${answer} does not.`,visual:{type:'spatial_cuboid',dimensions,points,candidateLabels:candidateCoordinates.map(point=>labelByKey.get(key(point))),caption:stretch===1?'Cube with labelled vertices':'Square-based prism with labelled vertices'}};
  }),
  composite_area_visual: gen('composite_area_visual','geom_area_composite_visual',(r,d)=>{
    const family=pick(r,['frame','notch','tiles']);
    if(family==='frame'){
      const width=integer(r,8,size(d,12,16,20)),height=integer(r,6,size(d,10,14,17)),left=integer(r,1,Math.max(1,Math.floor(width/3))),right=integer(r,1,Math.max(1,Math.floor(width/3))),top=integer(r,1,Math.max(1,Math.floor(height/3))),bottom=integer(r,1,Math.max(1,Math.floor(height/3))),innerW=width-left-right,innerH=height-top-bottom,answer=width*height-innerW*innerH;
      return {prompt:'The blue region is a rectangular border. What is its area?',answer,distractors:[width*height,innerW*innerH,2*(width+height),answer+innerW],formatter:v=>`${display(v)} cm²`,explanation:`Outer area ${width} × ${height} minus inner area ${innerW} × ${innerH} gives ${answer} cm².`,visual:{type:'composite_area',family,polygons:[{points:[[0,0],[width,0],[width,height],[0,height]]},{points:[[left,top],[left+innerW,top],[left+innerW,top+innerH],[left,top+innerH]],cutout:true}],dimensionLabels:[{x:width/2,y:height+.8,text:`${width} cm`},{x:-.3,y:height/2,text:`${height} cm`},{x:left+innerW/2,y:top+innerH/2,text:`${innerW} × ${innerH} cm`} ]}};
    }
    if(family==='notch'){
      const width=integer(r,7,size(d,11,15,18)),height=integer(r,6,size(d,10,13,16)),cutW=integer(r,2,width-3),cutH=integer(r,2,height-3),corner=pick(r,['tr','br','bl','tl']),base={tr:[[0,0],[width-cutW,0],[width-cutW,cutH],[width,cutH],[width,height],[0,height]],br:[[0,0],[width,0],[width,height-cutH],[width-cutW,height-cutH],[width-cutW,height],[0,height]],bl:[[0,0],[width,0],[width,height],[cutW,height],[cutW,height-cutH],[0,height-cutH]],tl:[[cutW,0],[width,0],[width,height],[0,height],[0,cutH],[cutW,cutH]]}[corner],answer=width*height-cutW*cutH;
      return {prompt:'A rectangular corner has been cut from the blue shape. Find the remaining area.',answer,distractors:[width*height,cutW*cutH,2*(width+height),answer+cutW],formatter:v=>`${display(v)} cm²`,explanation:`${width} × ${height} − ${cutW} × ${cutH} = ${answer} cm².`,visual:{type:'composite_area',family,polygons:[{points:base}],dimensionLabels:[{x:width/2,y:height+.8,text:`outer width ${width} cm`},{x:-.3,y:height/2,text:`${height} cm`},{x:width/2,y:-.5,text:`cut-out ${cutW} × ${cutH} cm`} ]}};
    }
    const rows=integer(r,3,size(d,5,7,8)),cols=integer(r,4,size(d,6,8,9)),cellSize=pick(r,[1,2,3]),path=[];for(let row=0;row<rows;row+=1){const columns=Array.from({length:cols},(_,i)=>row%2?cols-1-i:i);columns.forEach(col=>path.push([row,col]));}const shadedCount=integer(r,Math.ceil(path.length*.28),Math.floor(path.length*.72)),start=integer(r,0,path.length-shadedCount),shaded=path.slice(start,start+shadedCount),answer=shadedCount*cellSize*cellSize;
    return {prompt:'Find the area of the connected blue region.',answer,distractors:[shadedCount,rows*cols*cellSize*cellSize,(rows*cols-shadedCount)*cellSize*cellSize,shadedCount*cellSize],formatter:v=>`${display(v)} cm²`,explanation:`There are ${shadedCount} blue squares, each with area ${cellSize*cellSize} cm², so the area is ${answer} cm².`,visual:{type:'composite_area',family,rows,cols,cellSize,shaded}};
  }),
  net_to_open_box_volume: gen('net_to_open_box_volume','geom_nets',(r,d)=>{
    const baseWidth=integer(r,2,size(d,5,7,9)),baseDepth=integer(r,2,size(d,4,6,8)),wallHeight=integer(r,1,size(d,2,3,3)),unitLength=pick(r,d==='easy'?[1,2]:[1,2,3,4]),answer=baseWidth*baseDepth*wallHeight*unitLength**3;
    return {prompt:'This net folds to make an open box. What will the volume of the box be?',answer,distractors:[baseWidth*baseDepth*unitLength**2,(baseWidth+2*wallHeight)*(baseDepth+2*wallHeight)*unitLength**2,baseWidth*baseDepth*wallHeight*unitLength**2,2*(baseWidth+baseDepth)*wallHeight*unitLength**2],formatter:v=>`${display(v)} cm³`,explanation:`The box is ${baseWidth*unitLength} cm by ${baseDepth*unitLength} cm by ${wallHeight*unitLength} cm, so its volume is ${answer} cm³.`,visual:{type:'open_box_net',baseWidth,baseDepth,wallHeight,unitLength}};
  }),
  complete_shape_from_symmetry: gen('complete_shape_from_symmetry','geom_symmetry_completion',(r,d)=>{
    const bends=integer(r,0,size(d,1,2,3)),startY=.55+r()*.3,endX=.55+r()*.3,points=[[0,startY]];for(let i=1;i<=bends;i+=1){const t=i/(bends+1);points.push([endX*t,startY*(1-t)+Math.sin(Math.PI*t)*(r()-.5)*.09]);}points.push([endX,0]);const answer=4*(points.length-1);
    return {prompt:'The two dashed lines are lines of symmetry. When the blue part is reflected in both lines, how many sides will the completed shape have?',answer,distractors:[answer/2,answer+4,answer-2,points.length*4],explanation:`Each quadrant contributes ${points.length-1} sides, so the completed shape has 4 × ${points.length-1} = ${answer} sides.`,visual:{type:'symmetry_completion',quadrantPath:points}};
  }),
  grid_shape_packing: gen('grid_shape_packing','geom_grid_packing',(r,d)=>{
    const dimensions=pick(r,d==='easy'?[[4,4],[3,5]]:[[4,5],[3,6],[4,6],[5,5]]),[rows,cols]=dimensions,path=[];for(let row=0;row<rows;row+=1){const columns=Array.from({length:cols},(_,i)=>row%2?cols-1-i:i);columns.forEach(col=>path.push([row,col]));}const total=rows*cols;let cuts;do{const a=integer(r,2,total-6),b=integer(r,a+2,total-4),c=integer(r,b+2,total-2);cuts=[0,a,b,c,total];}while(cuts.some((v,i)=>i&&v-cuts[i-1]<2));const pieces=[];for(let i=0;i<4;i+=1)pieces.push({label:'',cells:path.slice(cuts[i],cuts[i+1])});const sizes=pieces.map(p=>p.cells.length);let extraSize;do{extraSize=integer(r,2,Math.min(8,total-2));}while(sizes.includes(extraSize));const extraPath=[];for(let i=0;i<extraSize;i+=1)extraPath.push(i<Math.ceil(extraSize/2)?[0,i]:[1,Math.ceil(extraSize/2)-1-(i-Math.ceil(extraSize/2))]);pieces.push({label:'',cells:extraPath});const arranged=shuffle(r,pieces),labels=['A','B','C','D','E'];arranged.forEach((piece,index)=>piece.label=labels[index]);const answer=arranged.find(piece=>piece===pieces[4]).label;
    return {prompt:'Four of the five pieces fit together without overlaps to cover the target rectangle exactly. Which piece is not needed?',answer,distractors:labels.filter(label=>label!==answer),explanation:`The target has ${total} squares. The four required pieces have ${sizes.join(', ')} squares, totalling ${total}; piece ${answer} has a different area and is the extra piece.`,visual:{type:'packing_puzzle',rows,cols,pieces:arranged}};
  }),
  tessellation_can_tile: gen('tessellation_can_tile','geom_tessellation',(r)=>{
    const tilers=shuffle(r,[{name:'equilateral triangle',sides:3},{name:'square',sides:4},{name:'rectangle',sides:4},{name:'parallelogram',sides:4},{name:'rhombus',sides:4},{name:'regular hexagon',sides:6}]).slice(0,4),non=pick(r,[{name:'regular pentagon',sides:5},{name:'regular heptagon',sides:7},{name:'regular octagon',sides:8},{name:'regular nonagon',sides:9},{name:'regular decagon',sides:10}]),shapes=shuffle(r,[...tilers,non]);
    return {prompt:'Which displayed shape cannot tessellate a flat surface using copies of itself only?',answer:non.name,distractors:tilers.map(x=>x.name),explanation:`A ${non.name} has an interior angle that does not fit a whole number of times around a point. The other displayed shapes can tessellate by themselves.`,visual:{type:'tessellation_choices',shapes}};
  }),
  surface_area_painted_fraction: gen('surface_area_painted_fraction','geom_surface_area',(r,d)=>{
    const side=integer(r,2,size(d,6,9,12)),subdivisions=pick(r,[2,3]),totalCells=6*subdivisions**2,denominator=pick(r,subdivisions===2?[2,3,4,6]:[2,3,6,9]),numerator=integer(r,1,denominator-1),paintedCount=totalCells*numerator/denominator,askUnpainted=r()<.5,area=6*side*side,answer=area*(askUnpainted?(denominator-numerator):numerator)/denominator,cells=[];for(let f=0;f<6;f+=1)for(let row=0;row<subdivisions;row+=1)for(let col=0;col<subdivisions;col+=1)cells.push([f,row,col]);const paintedCells=shuffle(r,cells).slice(0,paintedCount);
    return {prompt:`The net shows exactly ${fractionLabel(numerator,denominator)} of a cube's surface painted red. What area is ${askUnpainted?'unpainted':'painted'}?`,answer,distractors:[area-answer,side**3*numerator/denominator,area,side*side*numerator/denominator],formatter:v=>`${display(v)} cm²`,explanation:`The total surface area is 6 × ${side}² = ${area} cm². The ${askUnpainted?'unpainted':'painted'} fraction gives ${answer} cm².`,visual:{type:'cube_surface_net',side,subdivisions,paintedCells}};
  }),
  volume_composite_cubes: gen('volume_composite_cubes','geom_volume',(r,d)=>{
    const kind=pick(r,d==='easy'?['stairs','mountain']:['stairs','mountain','terraces','twin_peaks']),length=integer(r,5,size(d,7,9,11)),peak=integer(r,2,size(d,4,5,6));let profile;if(kind==='stairs')profile=Array.from({length},(_,i)=>1+Math.min(i,peak-1));else if(kind==='mountain')profile=Array.from({length},(_,i)=>1+Math.min(i,length-1-i,peak-1));else if(kind==='terraces')profile=Array.from({length},(_,i)=>1+Math.min(Math.floor(i/2),peak-1));else{const middle=Math.floor(length/2);profile=Array.from({length},(_,i)=>i===middle?1:1+Math.min(i,Math.abs(i-middle)-1,length-1-i,peak-1));}const heights=[profile],count=profile.reduce((a,b)=>a+b,0),mode=pick(r,['side','volume','surface']),side=integer(r,1,d==='hard'?3:2),cubeVolume=side**3,answer=count*cubeVolume,prompt=mode==='side'?`The shape is made from identical cubes with side length ${side} cm. What is its volume?`:mode==='volume'?`The shape is made from identical cubes. Each small cube has volume ${cubeVolume} cm³. What is the shape's volume?`:`Each small cube has surface area ${6*side*side} cm². What is the volume of the whole shape?`;
    return {prompt,answer,distractors:[count*side,count*6*side*side,(count-1)*cubeVolume,(count+1)*cubeVolume],formatter:v=>`${display(v)} cm³`,explanation:`There are ${count} cubes. Each has volume ${side}³ = ${cubeVolume} cm³, giving ${count} × ${cubeVolume} = ${answer} cm³.`,visual:{type:'cube_stack',heights,cubeSide:side,caption:`${kind==='mountain'?'Symmetrical stepped stack':kind==='terraces'?'Rising terraces':kind==='twin_peaks'?'Twin stepped towers':'Rising staircase'} of identical cubes`}};
  }),
